...
public class ChatActivity extends AppCompatActivity {
    ...
    private void processIndividualMessage(JSONObject messageObject) throws JSONException {
        ...
        String timestamp = messageObject.getString("timestamp"); // Get timestamp as String

        if (!receivedMessageIds.contains(messageId)) {
            boolean isSentByUser = (senderId == userId);
            Message message = new Message(text, isSentByUser, senderId, messageId, timestamp);
    ...
    private void displaySystemMessage(String systemMessage) {
        Message message = new Message(systemMessage, false, -1, -1, ""); // No timestamp for system messages
        messageList.add(message);
        messageAdapter.notifyItemInserted(messageList.size() - 1);
        chatRecyclerView.scrollToPosition(messageList.size() - 1);
    }
...
// I think this is Lucas's code, I added some things to it
